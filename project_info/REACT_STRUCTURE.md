...
Instead of a local hook, we use a centralized Redux store to manage the password lifecycle. This allows the input and feedback components to stay perfectly synced without prop drilling.
...
4.  **Subscribe**: Both components (`Input`, `Feedback`) receive the new state and re-render instantly.
